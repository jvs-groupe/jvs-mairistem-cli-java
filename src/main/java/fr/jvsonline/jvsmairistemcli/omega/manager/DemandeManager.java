...
public class DemandeManager extends BaseManager {
	...
	public List<DemandeModel> find() {
		...
		List<DemandeModel> myCollection = null;

		try {
			Settings omegaSettings = Settings.getInstance();
			Invocation.Builder invocationBuilder = this.client.getClient(omegaSettings.getWsCrossroad() + "/request", this.parameters);

			ResourceConverter converter = new ResourceConverter(DemandeModel.class); // convertisseur en JSONApi !
			...
			JSONAPIDocument<List<DemandeModel>> reqDocumentCollection = converter.readDocumentCollection(rawResponse, DemandeModel.class);
			myCollection = reqDocumentCollection.get();
		} catch (Exception e) {
	...
	public DemandeModel getById(Integer p_id) {
		...
		Invocation.Builder invocationBuilder = this.client.getClient("partner/request/" + p_id, this.parameters);
		...
		JSONAPIDocument<DemandeModel> myDocument = converter.readDocument(rawResponse, DemandeModel.class);
	...
	public DemandeModel sendReleve(ReleveModel p_reading) {
		...
		try {
			Invocation.Builder invocationBuilder = this.client.getClient("partner/request/reading", this.parameters);
			...
			String strResponse = response.readEntity(String.class);
			byte[] rawResponse = strResponse.getBytes();
			JSONAPIDocument<DemandeModel> bookDocument = converter.readDocument(rawResponse, DemandeModel.class);
	...
	public DemandeModel sendFactureExterne(FactureExterneModel p_bill) {
		...
		DemandeModel request = null;
		try {
			Invocation.Builder invocationBuilder = this.client.getClient("partner/request/bill", this.parameters);
			ResourceConverter converter = new ResourceConverter(FactureExterneModel.class,
					PointDeConsommationModel.class, DemandeModel.class);
			converter.enableDeserializationOption(DeserializationFeature.ALLOW_UNKNOWN_INCLUSIONS);
	...
	public DemandeModel updatePersonne(PersonneModel p_personne) {
		...
		DemandeModel request = null;
		try {
			Invocation.Builder invocationBuilder = this.client
					.getClient("partner/request/personne/" + p_personne.getId(), this.parameters);
